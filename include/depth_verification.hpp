...
DepthVerification::DepthVerification() : points(2, {-1,-1}), worldPoint(Eigen::Vector4d::Zero()) {}

// Parse camera model, intrinsic params, T_cn_cnm1
// Distortion, Resolution and RosTopic not parsed
...
void DepthVerification::start(){
    ...
    // int r = images[0].rows, c=images[0].cols;
    // double x[] = {c/10, c/2, 9*c/10, c/2}; //left, up, right, down
    // double y[] = {r/2, r/10, r/2, 9*r/10};
    // for(int i=0; i<4; i++){
    // Eigen::Matrix<double, 4, 1> worldPoint;
    // Eigen::Matrix<double, 2, 1> a;
    //     a = {x[i], y[i]};
    //     cams_[1].unproject(a, worldPoint);
    //     std::cout<<x[i] <<" "<< y[i]<<std::endl;
    //     std::cout<<worldPoint.transpose()<<std::endl;
    // }
